...
class State():
    ...
    def __init__(self, program, choice=None, stack=[]):
        ...
        Arguments:
            program {[list]} -- the list of instruction we want to compute space state # noqa

        Keyword Arguments:
            choice {[int]} -- number to choose which instruction to execute (default: {None}) # noqa
    ...
    def evaluation(self):
        ...
        if not(self.program):
            return [self]
        # if await instruction we loop over every possible choice
        # and the await loop isn't empty
        ...
        # if the await loop is empty
            ...
            return [State(
                self.program[1:], None, copy(self.stack) + [self.program[0]]
                )]
...
class FixPoint():
    ...
    def run(self):
        ...
        while not(end):
            ...
            # if the copy is equal to the new,
            # it means we reach a fix poit, so stop the loop
